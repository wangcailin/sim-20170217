$(function(){
	...
	if ($('.content').attr('data-url') === 'index') {
		...
		$('.footer-click a').on('touchstart',function(){
			...
			$(this).attr('href','/external/sim-20180217/index.php?m=Event&a=picture' + '#id=' + randomArr[mySwiper.activeIndex-1].hier + '/' + randomArr[mySwiper.activeIndex-1].level)
		})
		$('.arrowBottom').click(function(){
			mySwiper.slideNext();
		})
        var onlineAudio = document.getElementById('audioLone');
        $('body').on(
            'touchstart', function(){
                onlineAudio.play();
            }
    ...
    var allAudio = document.getElementById('audio');
    var musicFalg = true;
