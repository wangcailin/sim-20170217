...
userScale = typeof(userScale)!='undefined' ? userScale : 0;
userScale='no';
...
if (/Android (\d+\.\d+)/.test(ua)){
	...
	// andriod 2.3
	if(version>2.3){
		viewportContent = 'width='+contentWidth+', initial-scale='+scale+', minimum-scale = '+minScale+', maximum-scale = '+maxScale+', target-densitydpi=device-dpi, user-scalable='+userScale;
		// andriod 2.3
		function orientationChange()
		{
			setTimeout(function(){
				...
				document.querySelector("meta[name=viewport]").setAttribute('content','width='+contentWidth+',initial-scale='+scale+', minimum-scale='+minScale+', maximum-scale='+maxScale+', target-densitydpi=device-dpi, user-scalable='+userScale);
			},500);
		}
		window.onorientationchange = orientationChange;
	}else{
		viewportContent = 'width='+contentWidth+', user-scalable='+userScale+', target-densitydpi=device-dpi';
	}
} else {
	viewportContent = 'width='+contentWidth+', user-scalable='+userScale+', target-densitydpi=device-dpi';
}
