...
function is_contains_emoji ($text) {
	
	$emojis = array_keys($GLOBALS['emoji_maps']);
	
	for ($i=0; $i<count($emojis); $i++) {
		
		if (false !== strpos($text, $emojis[$i])) {
...
function replace_null_emoji ($text) {
	
	$emojis = array_keys($GLOBALS['emoji_maps']);
	
	for ($i=0; $i<count($emojis); $i++) {
		$text = str_replace($emojis[$i],'',$text);
	}
