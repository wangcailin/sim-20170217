...
class Wechatext
{
	...
	public function send($id,$content)
	{
        ...
        $send_snoopy->referer = "https://mp.weixin.qq.com/cgi-bin/singlemsgpage?fromfakeid={$id}&msgid=&source=&count=20&t=wxm-singlechat&lang=zh_CN";
								// https://mp.weixin.qq.com/cgi-bin/singlesendpage?tofakeid={$id}&t=message/send&action=index&token={this->_token}&lang=zh_CN
	 ...
	 * @return array
	 */
	function getUserlist($page=0,$pagesize=10,$gid=0){
		...
		$send_snoopy->rawheaders['Cookie']= $this->cookie;
		$submit = "https://mp.weixin.qq.com/cgi-bin/contactmanage?t=user/index&pagesize=".$pagesize."&pageidx=".$page."&type=0&groupid=".$gid."&token=".$this->_token."&lang=zh_CN";
		//echo $submit;die;
	...
	public function sendNews($id,$msgid)
	{
		...
		$post['error'] = 'false';
		$post['ajax'] = 1;
        $send_snoopy->referer = "https://mp.weixin.qq.com/cgi-bin/singlesendpage?tofakeid={$id}&t=message/send&action=index&token={$this->_token}&lang=zh_CN";									
		$send_snoopy->rawheaders['Cookie']= $this->cookie;
		$submit = "https://mp.weixin.qq.com/cgi-bin/singlesend?t=ajax-response";
		$send_snoopy->submit($submit,$post);
		$this->log($send_snoopy->results);
	...
	/**
	 * 发送图文信息,必须从图文库里选取消息ID发送
	 * @param  string $gid     组的id
	 * @param  string $fakeId  用户的fakeid列表 | 分隔
	 */
	public function sendgoup($gid,$fakeId)
	{
		$send_snoopy = new Snoopy; 
		$post = array();
		$post['token'] = $this->_token;
        $post['action'] = 'modifycontacts';
        $post['contacttype'] = $gid;
        $post['t'] = 'ajax-putinto-group';
        $post['tofakeidlist'] = str_replace(chr(13).chr(10),'|',$fakeId);
        $send_snoopy->referer = "https://mp.weixin.qq.com/cgi-bin/singlemsgpage?fromfakeid={$id}&msgid=&source=&count=20&t=wxm-singlechat&lang=zh_CN";
		$send_snoopy->rawheaders['Cookie']= $this->cookie;
		$submit = "https://mp.weixin.qq.com/cgi-bin/modifycontacts";
		$send_snoopy->submit($submit,$post);
		$this->log($send_snoopy->results);
		return $send_snoopy->results;
	}
	...
	public function getInfo($id)
	{
		...
		$this->log($send_snoopy->results);
		preg_match("/to_nick_name([^,]*),/", $send_snoopy->results,$matchs);
		$str=$matchs[1];
		$str=str_replace('"', "", $str);
		$str=str_replace(':', "", $str);
		return $str;
	 ...
	 */
	public function getResouseList($page=1)
	{
		$start=($page-1)*50;
		$send_snoopy = new Snoopy;
		$send_snoopy->rawheaders['Cookie']= $this->cookie;
		$send_snoopy->referer = "https://mp.weixin.qq.com/cgi-bin/message?t=message/list&lang=zh_CN&count=50&token=".$this->_token;
		$url = "https://mp.weixin.qq.com/cgi-bin/appmsg?begin=$start&count=50&t=media/appmsg_list&type=10&action=list&token=".$this->_token."&lang=zh_CN";
		$send_snoopy->fetch($url);
		$result = $send_snoopy->results;
	...
	public function getAllResponse(){
		//https://mp.weixin.qq.com/cgi-bin/autoreply?t=ivr/keywords&action=smartreply&token=665347804&lang=zh_CN
		$send_snoopy = new Snoopy;
		$send_snoopy->rawheaders['Cookie']= $this->cookie;
		$send_snoopy->referer = "https://mp.weixin.qq.com/cgi-bin/message?t=message/list&lang=zh_CN&count=50&token=".$this->_token;
		$url = "https://mp.weixin.qq.com/cgi-bin/autoreply?t=ivr/keywords&action=smartreply&token=".$this->_token."&lang=zh_CN";
		$send_snoopy->fetch($url);
		$result = $send_snoopy->results;
		
		preg_match("/list : \[.*\};/is",$result,$matches);
	...
	public function login(){
		...
		if ($result['base_resp']['ret']!=0) return false;
